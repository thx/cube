...
「normalize」的核心理念是不重置为0，让元素拥有统一的默认间距，大小等表现。但针对国内大部分网站不是纯文字排版，neat.css 选择回归「有即是无，无即是有」的理念，把大部分标签的默认`margin`，`padding` 均重置为 0。如果你需要对大面积文字或者文档快速美化，推荐单独引入专门针对汉字排版的[type.css](#)。
...
neat.css 目前压缩后只有 4KB，对于用不到的样式对页面也不会造成影响。而且实际开发中我们并不能确定页面中会增加或减少什么元素，所以并不推荐自行定制。
